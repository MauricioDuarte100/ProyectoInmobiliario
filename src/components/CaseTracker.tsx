...
export default memo(function CaseTracker({ events }: Props) {
  return (
    <div className="relative">
      <div className="absolute left-[22px] top-4 bottom-4 w-[2px] bg-gradient-to-b from-green-trust/40 via-blue-tech/30 to-night/10" />
        ...
        {events.map((event, i) => {
          ...
          return (
            <li key={i} className="relative flex gap-5">
              <div className="relative z-10 mt-1">
                {isLast ? (
                  <span className="relative flex h-5 w-5">
                    <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-green-trust opacity-60" />
                    <span className="relative inline-flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-green-trust to-green-trust/60 shadow-lg shadow-green-trust/40">
              ...
              <div className={`flex-1 rounded-xl border border-border/60 bg-white/70 backdrop-blur px-4 py-3.5 ${isLast ? 'shadow-md border-green-trust/20' : ''}`}>
                ...
                <h4 className="flex items-center gap-1.5 text-sm font-bold text-text-primary">
                  <FileText className="h-3.5 w-3.5 text-green-trust" />
                  {event.title}
                </h4>
